...
public class ASTBuilder extends BadlangBaseVisitor<Object> {
	...
	public Object visitExprStmt(BadlangParser.ExprStmtContext ctx) {
		...
		// Assignment is parsed as an expression, but represented as a Stmt.Assign.
		// We check for that case here and return the statement directly.
		if (v instanceof Stmt)
			return (Stmt) v; // assignment case becomes a statement, there was not an Expr.Assign
	...
	public Object visitBlock(BadlangParser.BlockContext ctx) {
		...
		for (ParseTree child : ctx.children) {
			if (child instanceof BadlangParser.DeclarationContext || child instanceof BadlangParser.StatementContext) {
	...
	public Object visitAssignment(BadlangParser.AssignmentContext ctx) {
		if (ctx.logic_or() != null) {
			...
			// Badlang assignment does not return a value, so it's represented as a statement.
