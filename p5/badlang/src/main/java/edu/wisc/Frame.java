...
public final class Frame {
    ...
    private Frame(int frameSize) { this.frameSize = frameSize; }
    ...
    private static int countLocalsFlat(java.util.List<Stmt> stmts) {
        int n = 0;
        for (var s : stmts) {
            if (s instanceof Stmt.Var) n++;
            else if (s instanceof Stmt.Block b) n += countLocalsFlat(b.statements);
            else if (s instanceof Stmt.If i) {
                n += countLocalsFlat(List.of(i.thenBranch));
                if (i.elseBranch != null) n += countLocalsFlat(List.of(i.elseBranch));
    ...
    private static void assignLocalsFlat(java.util.List<Stmt> stmts, Map<String,Integer> map, int startOff) {
        int off = startOff;
        for (var s : stmts) {
            if (s instanceof Stmt.Var v) { map.put(v.name, off); off -= 4; }
            else if (s instanceof Stmt.Block b) assignLocalsFlat(b.statements, map, off);
            else if (s instanceof Stmt.If i) {
                assignLocalsFlat(List.of(i.thenBranch), map, off);
                if (i.elseBranch != null) assignLocalsFlat(List.of(i.elseBranch), map, off);
            } else if (s instanceof Stmt.While w) {
